...
## 入っているもの
- `index.html` … トップページ（構想・ピントダイヤル・7ステージ地図）
- `stage-1.html` … Stage 1「Webの体」（中身×見た目の分離を体感するラボ）

依存パッケージ・ビルド作業は一切なし。ファイルをそのまま置けば動きます。
...
3. 作成直後の画面で **uploading an existing file** のリンクをクリック。
   - `index.html` と `stage-1.html` を**ドラッグ＆ドロップ**でアップロード。
